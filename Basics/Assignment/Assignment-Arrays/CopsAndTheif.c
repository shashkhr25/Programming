...
int main(){
	...
	//Check the indezing of this once more. off by 1 digit someplace.
	...
	for(i=1;i<=noOfTestCases;i++){
		int M, x, y, noOfSafeHouses;
		printf("Enter no of houses where cops live: ");
		scanf("%d", &M);
		
		int copsHouses[M];
		for(j=0;j<M;j++){
			printf("Enter the index where %d cop lives : ", j+1);
		...
		for(j=0;j<M;j++){
			int first, last;
			first = (copsHouses[j] -k)-1;
			last = (copsHouses[j] + k)-1;
